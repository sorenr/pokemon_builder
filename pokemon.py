...
class Pokemon():
    ...
    def update(self,
        ...
        if self.is_shadow:
            self.bonus_atk *= self.settings.get(self.gm.K_SHADOW_BONUS_ATK, 1)
            self.bonus_def *= self.settings.get(self.gm.K_SHADOW_BONUS_DEF, 1)
        ...
        self.defense *= self.settings.get("defenseBonusMultiplier", 1)
                ...
                for s in range(0, 16):
                    for l in [x * 0.5 for x in range(2, 83)]:
                        ...
                        if (sp_optimal is None or sp_t > sp_optimal or
                            ...
                            sp_optimal = sp_t
                            cp_optimal = cp_t
                            optimal = [(a, d, s, l, cp_t, sp_t)]
                        elif sp_t == sp_optimal:
                            optimal.append((a, d, s, l, cp_t, sp_t))
        return optimal
    ...
    def optimize_moves(self, target=None, fast=True, charged=True):
        ...
        # print the ranking if we're printing debug messages
        if logging.DEBUG == logging.getLogger().getEffectiveLevel():
            for result in results:
                for combo in combos[result]:
                    logging.debug("{0:0.2f} {1:s} {2:s}".format(result, combo[0].name, combo[1].name))
